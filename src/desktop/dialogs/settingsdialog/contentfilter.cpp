...
void ContentFilter::initBuiltIn(desktop::settings::Settings &settings, QVBoxLayout *layout)
{
	...
	auto *level = form->addRadioGroup(tr("Filter mode:"), false, {
		...
		{ tr("Remove sessions from listings"), int(parentalcontrols::Level::NoList) },
		{ tr("Prevent joining filtered sessions"), int(parentalcontrols::Level::NoJoin) },
		{ tr("Disconnect when sessions become not safe for me"), int(parentalcontrols::Level::Restricted) }
	});
