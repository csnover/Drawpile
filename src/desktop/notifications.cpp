...
static bool isEnabled(Event event)
{
	...
	case Event::UNLOCKED:
		...
		return settings.hasNotificationUnlock()
			? settings.notificationUnlock()
			: settings.notificationLock();
	case Event::LOGIN:
		return settings.notificationLogin();
	case Event::LOGOUT:
		// Used to be the same value as login, so default to that if not found.
		return settings.hasNotificationLogout()
			? settings.notificationLogout()
