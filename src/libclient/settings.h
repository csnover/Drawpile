...
protected:
	...
	bool has(const SettingMeta &setting) const;

	template <
		typename T,
		typename Object,
		typename Receiver,
		typename Signal = nullptr_t,
		typename Self,
	...
	)
	{
		...
		if constexpr (std::is_invocable_v<Receiver, Object *, T>) {
			std::invoke(receiver, object, value);
		} else if constexpr (std::is_invocable_v<Receiver, Object *>) {
		...
		if constexpr (std::is_invocable_v<Signal, Object *, T>) {
			return std::pair {
				connection,
	...
	inline auto bindAs(T initialValue, void (Self::*changed)(T), void (Self::*setter)(T), Object *object, Receiver receiver, Signal signal = nullptr)
	{
		...
		if constexpr (std::is_invocable_v<Receiver, Object *, U>) {
		...
		if constexpr (std::is_invocable_v<Signal, Object *, U>) {
			return std::pair {
				connection,
