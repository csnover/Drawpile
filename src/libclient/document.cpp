...
void Document::sendResetSession(const drawdance::MessageList &resetImage)
{
	...
	if(resetImage.isEmpty()) {
		...
		qInfo("Sending session reset request. (Snapshot size is %lld bytes)", compat::castSize(resetImage.length()));
	}
...
void Document::sendResetSnapshot()
{
	...
	if(m_sessionHistoryMaxSize > 0 && m_resetstate.length() > m_sessionHistoryMaxSize) {
		qWarning("Reset snapshot (%lld) is larger than the size limit (%d)!", compat::castSize(m_resetstate.length()), m_sessionHistoryMaxSize);
