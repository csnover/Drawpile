...
void Document::sendResetSession(const drawdance::MessageList &resetImage)
{
	...
	if(resetImage.isEmpty()) {
		...
		qInfo("Sending session reset request. (Snapshot size is %lld bytes)", static_cast<long long>(resetImage.length()));
	}
...
void Document::sendResetSnapshot()
{
	...
	if(m_sessionHistoryMaxSize > 0 && m_resetstate.length() > m_sessionHistoryMaxSize) {
		qWarning("Reset snapshot (%lld) is larger than the size limit (%d)!", static_cast<long long>(m_resetstate.length()), m_sessionHistoryMaxSize);
