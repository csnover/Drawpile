...
bool NewVersionCheck::needCheck(libclient::settings::Settings &settings)
{
	...
	const QDate lastSuccess = QDate::fromString(settings.versionCheckLastSuccess(), Qt::ISODate);
	...
	if(lastSuccess == lastCheck)
		...
		if(daysSinceLastSuccess > 30) {
			settings.setVersionCheckEnabled(false);
			return false;
		}
...
// Normative comments here are descriptions from the semver specification
...
void NewVersionCheck::querySuccess(libclient::settings::Settings &settings)
{
	...
	settings.setVersionCheckLastCheck(now);
	settings.setVersionCheckLastSuccess(now);
