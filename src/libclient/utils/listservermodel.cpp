...
QVariant ListServerModel::data(const QModelIndex &index, int role) const
{
	if(index.isValid() && index.row() >= 0 && index.row() < m_servers.size()) {
		...
		case Qt::ToolTipRole: return tr("%1\n\n%2\n\nURL: %3\nRead-only: %4, public: %5, private: %6").arg(
			srv.name,
			srv.description,
			srv.url,
			srv.readonly ? tr("yes", "readonly") : tr("no", "readonly"),
			srv.publicListings ? tr("yes", "public") : tr("no", "public"),
			srv.privateListings ? tr("yes", "private") : tr("no", "private")
			);
